...
ImageDirectory_dur="SPEI_SRFI_SWSI/duration_DZD/"
ImageDirectory_wait="SPEI_SRFI_SWSI/waitingtimeDZD/"
...
ToFE_DZD_event_nc   <- nc_open("SPEI_SRFI_SWSI/ToFE_DZD_event/ToFE_DZD_event.nc")

ToFE_DZD_event     <- ncvar_get(ToFE_DZD_event_nc,varid="ToFE")

maxlength <- 3012

for (nens in 1:100) {
  boolean_DZD_event_nc    <- nc_open(paste("SPEI_SRFI_SWSI/DZD_event/DZD_event_spei4815_srfi4815_swsi4806_TRD.",as.character(nens),".nc",sep=""))
  
  boolean_DZD_event       <- ncvar_get(boolean_DZD_event_nc,varid='event')
  
  waitingTime_DZD_event    <- array(NA,c(nlons,nlats,3012))
  duration_DZD_event       <- array(NA,c(nlons,nlats,3012))
  
  mean_waitingTime_DZD_event    <- array(NA,c(nlons,nlats))
  mean_duration_DZD_event       <- array(NA,c(nlons,nlats))
  
  ################################################
  ######  Waiting time DZD_event
  #########################################
  
  for (jlon in 1:nlons){
    for (jlat in 1:nlats){
      if (landfrac[jlon,jlat]>0.5 && !is.na(ToFE_DZD_event[jlon,jlat])){
        
        year_ToFE <- ToFE_DZD_event[jlon,jlat]
        ts_bool  <- ts(boolean_DZD_event[jlon,jlat,],start=c(1850,01),frequency=12)
        data     <- window(ts_bool,start = c(year_ToFE),frequency=12)
        ...
        waitingTime_DZD_event[jlon,jlat,]   <- c(new_waittime,rep(NA, maxlength - length(new_waittime)))
        mean_waitingTime_DZD_event[jlon,jlat] <- mean(new_waittime)
        #print(jlon)
      }
    }
  }
  ...
  for (jlon in 1:nlons){
    for (jlat in 1:nlats){
      if (landfrac[jlon,jlat]>0.5 && !is.na(ToFE_DZD_event[jlon,jlat])){
        ...
        duration_DZD_event[jlon,jlat,]    <- c(duration,rep(NA, maxlength - length(duration)))
        mean_duration_DZD_event[jlon,jlat] <- mean(duration)
        #print(jlon)
      }
    }
  }
  ...
  ##############################$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$##################
  ##### save mean waiting time and Duration to netcdf
  ##############################$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$##################
  ...
  ncout1 <- nc_create(paste(ImageDirectory_dur,"mean_duration_DZD_event.",as.character(nens),".nc", sep = ""), list(var_prec1),force_v4=TRUE)
  ncout2 <- nc_create(paste(ImageDirectory_wait,"mean_waitingTime_DZD_event.",as.character(nens),".nc", sep = ""), list(var_prec2),force_v4=TRUE)

  ncvar_put(ncout1, var_prec1, mean_duration_DZD_event, start=c(1,1), count=c(nx,ny), verbose=TRUE)
  ncvar_put(ncout2, var_prec2, mean_waitingTime_DZD_event, start=c(1,1), count=c(nx,ny), verbose=TRUE)
  ...
  title        <- ncatt_get(boolean_DZD_event_nc,0,"DZD Duration and Waiting Time")
  institution  <- ncatt_get(boolean_DZD_event_nc,0,"institution")
  datasource   <- ncatt_get(boolean_DZD_event_nc,0,"source")
  references   <- ncatt_get(boolean_DZD_event_nc,0,"references")
  history      <- ncatt_get(boolean_DZD_event_nc,0,"history")
  Conventions  <- ncatt_get(boolean_DZD_event_nc,0,"Conventions")
  ...
  ##############################$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$##################
  ##### save waiting time and Duration time series to netcdf
  ##############################$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$##################
  
  ########
  time        <- ncvar_get(boolean_DZD_event_nc, "time")
  tunits      <- ncatt_get(boolean_DZD_event_nc,"time","units")
  calendar    <- ncatt_get(boolean_DZD_event_nc,"time","calendar")$value
  ...
  ######Save to netcdf 
  ncout11 <- nc_create(paste(ImageDirectory_dur,"duration_DZD_event.",as.character(nens),".nc", sep = ""), list(var_prec11),force_v4=TRUE)
  ncout12 <- nc_create(paste(ImageDirectory_wait,"waitingTime_DZD_event.",as.character(nens),".nc", sep = ""), list(var_prec12),force_v4=TRUE)
  
  ncvar_put(ncout11, var_prec11, duration_DZD_event, start=c(1,1,1), count=c(nx,ny,nt), verbose=TRUE)
  ncvar_put(ncout12, var_prec12, waitingTime_DZD_event, start=c(1,1,1), count=c(nx,ny,nt), verbose=TRUE)
  ...
  # get global attributes
  title        <- ncatt_get(boolean_DZD_event_nc,0,"DZD")
  institution  <- ncatt_get(boolean_DZD_event_nc,0,"institution")
  datasource   <- ncatt_get(boolean_DZD_event_nc,0,"source")
  references   <- ncatt_get(boolean_DZD_event_nc,0,"references")
  history      <- ncatt_get(boolean_DZD_event_nc,0,"history")
