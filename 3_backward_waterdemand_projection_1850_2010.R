...
for (ens in 1:length(model)){
  ...
  Twat_model_nc          = nc_open(paste(as.character(model[ens]),'/total_consumption_',as.character(ssps[nssp]),'_',as.character(model[ens]),'_sectors_monthly.nc',sep=''))
  ...
  pop_past_nc          <- nc_open(paste("total_waterDeman_models/water_consumption/past_total_water_consumption_",as.character(ssps[nssp]),"_miroc_sectors_monthly_1850_2009.nc",sep=""))
  ...
  ncout1 <- nc_create(paste0(as.character(model[ens]),'/past_total_water_demand_',as.character(ssps[nssp]),'_',as.character(model[ens]),'_sectors_monthly_1850_2010_Log_liter_time_rev.nc',sep=''), list(var_prec1),force_v4=TRUE)
